...
pub enum Backend {
    ...
    /// once any provider is configured (Anthropic, OpenAI, Gemini, Monshoot, z.ai's GLM, or a local
    /// Ollama) — each turn calls that provider's chat API over the conversation transcript.
    HarnessAdi,
