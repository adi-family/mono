...
use crate::types::{
    ...
    SaveEmbeddingBackend, SaveEmbeddingSettings,
};
...
pub fn save_embedding_backend(store: &Agents, body: &[u8]) -> Response {
    ...
    // `candle`/`hash` never take a model or width from configuration — the panel and the CLI both
    // leave those fields out of the form entirely for these two runtimes, so whatever arrived here
    // is stale or blank and the fixed pair always wins.
    ...
    if let Err(e) = registry.save(req.id.trim(), manifest) {
